...
-The remaining days of data for each user was transformed into the target variable under the same conditions discussed earlier, now applied to the 20 day timeline, to achieve a binary output variable with 1 indicating future active users and 0 indicating future inactive users. In mathematical terms, the below transformation was applied:
<div align="center"><img style="background: white;" src="https://render.githubusercontent.com/render/math?math=%5Cleft%20%5C%7B%20%0A%5Cbegin%7Barray%7D%7Bc%7D%0A1%2C%20if%20%5Csum_%7Bt%3D11%7D%5E30%20isClick%20%3E%200%20%5C%5C%20%0A0%2C%20otherwise%20%5C%5C%20%0A%5Cend%7Barray%7D%0A%5Cright."></div>

Hence, the goal of our predictive model was a binary classification of users into future active or future inactive based on the initial 10 days of summed data, with our class of interest being future active users and our goal being achieving the highest possible accuracy in this prediction. Common machine learning models for binary classification are Support Vector Machines (SVM) and Naïve Bayes (NB), and hence they were our first choices to test out. We also looked at Decision Trees (DT) as they are purportedly apt for general classification problems, and Neural Networks (NN) as they can result in high levels of accuracy at the cost of interpretability.
