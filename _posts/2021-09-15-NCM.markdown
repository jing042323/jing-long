---
layout: post
title: NetEase Cloud Music Case Study - Part I
date: 2021-09-15
...
- Models to predict user activity, including Logistic Regression, SVM, Random Forest and Neural Networks.  
...
<span style="color:Coral">**This post contains Section 1 and 2. Please see here for Section 3 and 4.**</span>
...
### <span style="color:Coral"> 2.5 Models to Predict User Activity Level from Initial Actions </span>
...
#### <span style="color:Coral"> 2.5.1 Model Preparation </span>
...
Hence, the goal of our predictive model is a binary classification of users into future active or future inactive based on the initial 10 days of data, with our class of interest being future active users and our goal being achieving the highest possible accuracy in this prediction. Common machine learning models for binary classification are Logistic Regression and Support Vector Machines (SVM), and hence they are our first choices to test out. We also look at Random Forest as they are purportedly apt for general classification problems, and Neural Networks (NN) as they can result in high levels of accuracy but at the cost of interpretability.
...
#### <span style="color:Coral"> 2.5.2 Logistics Regression </span>
...
The optimal hyperparameters we get from Grid Search is C = 0.01, penalty = L1 and solver = liblinear. After retraining the model with the best setting of hyperparameters and apply the optimal logistic regression to the test set, we get the following results:  
...
The accuracy score on test set is about 0.72, and the accuracy score for our class of insterest (active user) is higher than that of the inactive class. From the coefficient table above, we can find that sum of clicks and impressions have the largest magnitude of positive impact, implying that these actions are the most characteristic of active users. The variables registeredMonthCnt and age however show a negative impact but relatively small in magnitude. These are in line with our findings in the previous sections.

#### <span style="color:Coral"> 2.5.3 SVM </span>
...
#### <span style="color:Coral"> 2.5.4 Random Forest </span>
...
#### <span style="color:Coral"> 2.5.5 Neural Networks </span>
...
(i) Input layer: the input layer will consist of 47 neurons. We will ‘flatten’ the input data, so that a (28x28) shape flattens to 784.  
(ii) Output layer: the output layer will have 2 neurons, one per class.  
(iii) Hidden layer: we have decided to set 3 hidden layers. When deciding the number of neurons on hidden layers, we can consider the following rules: The number of hidden neurons should be between the size of the input layer and the output layer. The most appropriate number of hidden neurons is sqrt(input layer nodes * output layer nodes). The number of hidden neurons should keep on decreasing in subsequent layers to get more and more close to pattern and feature extraction and to identify the target class. According to these rules, we can get:
The number of hidden neurons is between 2 and 47.
The most appropriate number of hidden neurons is  sqrt(2*47) ≈ 10
...
Here, we will try stochastic gradient descent with batch size 1. And 3 different epochs which are 10, 100 and 1000. We will apply the training-validation-test procedure to find out the best value for epochs and batch_size.
